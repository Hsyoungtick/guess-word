...
import { createRoom, joinRoom, leaveRoom, listRooms, requestHint, requestRematch, resumeRoom, startGame, submitGuess } from '../api'
...
export default function Home() {
  ...
  const hint = snapshot?.room.hint
  const hintText = hint?.level ? `提示：${hint.category ? `分类是「${hint.category}」` : ''}${hint.length ? `，答案是 ${hint.length} 个字` : ''}` : '提示会逐步公开答案分类和字数'
  ...
  async function sendGuess(event: FormEvent) {
    ...
    if (!credential || !snapshot || !guess.trim() || snapshot.room.currentPlayerId !== me?.id) return
    const word = [...guess.trim()].slice(0, 8).join('')
    setGuess('')
    await command(() => submitGuess(credential, word, snapshot.room.version))
  }

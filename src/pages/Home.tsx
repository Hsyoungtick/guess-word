...
import { createRoom, joinRoom, leaveRoom, listRooms, requestRematch, startGame, submitGuess } from '../api'
...
function LobbyCard({ room, onJoin }: { room: LobbyRoom; onJoin: (room: LobbyRoom) => void }) {
  const canJoin = room.status === 'waiting' && room.playerCount < room.maxPlayers
  return <article className="lobby-card"><div><span className="room-code">{room.code}</span><span className={`room-status ${room.status}`}>{room.status === 'waiting' ? '等待中' : room.status === 'playing' ? '进行中' : '暂停中'}</span></div><h3>{room.category} · {room.difficulty}</h3><p>{room.playerCount}/{room.maxPlayers} 位玩家</p><button disabled={!canJoin} onClick={() => onJoin(room)}>{canJoin ? '加入房间' : '不可加入'}<ArrowRight size={16} /></button></article>
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    void refreshLobby(); const id = window.setInterval(() => void refreshLobby(), 5000)
  ...
  return <main className="app-shell"><div className="noise" aria-hidden="true" /><header className="topbar"><button className="brand" onClick={goHome}><span className="brand-mark"><Radio size={22} /></span><span>词频对决</span></button><div className="broadcast"><i /> {snapshot.room.code} · {network === 'online' ? 'ONLINE' : 'NETWORK CHECK'}</div></header><section className="game-stage"><div className="game-meta"><span>房间 {snapshot.room.code}</span><span>{players.length}/{snapshot.room.maxPlayers} 位玩家</span><span>{snapshot.room.category} · {snapshot.room.difficulty}</span><span className={`network-${network}`}>{network === 'online' ? '已连接' : network === 'connecting' ? '连接中' : network === 'unstable' ? '网络波动' : '离线'}</span></div>{snapshot.room.status === 'paused' && <div className="pause-banner">房间暂无在线玩家，五分钟后会自动销毁；若仍有玩家，系统会把房主转交给最早加入的人。</div>}{snapshot.room.status === 'waiting' && <div className="waiting-head"><div className="channel-card"><p className="eyebrow">PUBLIC CHANNEL</p><h1>{snapshot.room.code}</h1><p>最多 {snapshot.room.maxPlayers} 人 · {snapshot.room.category} · {snapshot.room.difficulty}</p><button type="button" onClick={() => void copyInvite()}><Copy size={17} />{copyStatus === '邀请链接已复制' ? '已复制' : '复制邀请链接'}</button>{copyStatus && <p className="copy-status" role="status">{copyStatus}</p>}</div><p className="waiting-note">{players.length < 2 ? '等待其他玩家接入频道…' : me?.isHost ? '信号已就绪，可以开始对决' : '等待房主开始对局'}</p>{me?.isHost && <button className="start-button waiting-start" disabled={busy || players.length < 2} onClick={() => credential && command(() => startGame(credential))}><span>{busy ? '正在抽取词语…' : '房主开始对决'}</span><ArrowRight /></button>}</div>}{snapshot.room.status === 'playing' && <section className="tuning-console"><div className="turn-banner">轮到 {current?.nickname || '下一位玩家'}<span className={`timer ${seconds <= 8 ? 'urgent' : ''}`}>{seconds}s</span></div><div className={`score-display ${(latest?.similarity || 0) >= 80 ? 'hot' : ''}`}><span className="score-label">{latest ? '最新猜词' : '等待首次猜词'}</span><div className="latest-word">{latest?.displayWord || '——'}</div><strong>{latest?.similarity || 0}<small>%</small></strong><div className="meter"><i style={{ width: `${latest?.similarity || 0}%` }} /></div><div className="meter-ticks"><span>微弱</span><span>接近</span><span>命中</span></div></div><form className="guess-form" onSubmit={sendGuess}><label htmlFor="guess">{snapshot.room.currentPlayerId === me?.id ? '输入你的猜测（1～8 字）' : `等待 ${current?.nickname || '玩家'} 发射信号`}</label><div><input id="guess" disabled={busy || snapshot.room.currentPlayerId !== me?.id || seconds <= 0} maxLength={8} value={guess} onChange={(e) => setGuess([...e.target.value].slice(0, 8).join(''))} placeholder="1～8 个字" /><button disabled={busy || snapshot.room.currentPlayerId !== me?.id || !guess.trim()}>发射</button></div></form></section>}{snapshot.room.status === 'finished' && <div className="winner-card"><Trophy size={42} /><p>本局胜者</p><h2>{players.find((p) => p.id === snapshot.room.winnerId)?.nickname}</h2><strong>成功锁定：{snapshot.room.answer}</strong><button disabled={busy || Boolean(me?.rematchReady)} onClick={() => credential && command(() => requestRematch(credential))}><RotateCcw size={18} />{me?.rematchReady ? '已确认，等待其他玩家' : '确认再来一局'}</button><small>{rematchReadyCount}/{players.length} 位玩家已确认</small></div>}{playerGrid}<section className="history-board"><div className="history-heading"><div><span>公开波段</span><h2>猜测记录</h2></div><span>共 {sortedHistory.length} 次信号</span></div>{sortedHistory.length ? <div className="history-list">{sortedHistory.map((item) => { const player = players.find((p) => p.id === item.playerId); return <div className={`history-item p${player?.seat || 1}`} key={item.id}><span className="history-player">{player?.nickname}</span><strong>{item.displayWord}</strong><b>{item.similarity}%</b></div> })}</div> : <div className="empty-history">频道还很安静，第一条猜测会出现在这里。</div>}</section><div className="room-actions"><button className="quit-button" onClick={goHome}>返回大厅</button><button className="quit-button danger" onClick={() => void leave()}>退出房间</button></div></section></main>
}

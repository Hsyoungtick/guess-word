...
export default function Home() {
  ...
  return <main className="app-shell"><div className="noise" aria-hidden="true" />
    ...
    : snapshot.room.status === 'waiting' ? <section className="waiting-stage"><div className="channel-card"><p className="eyebrow">PRIVATE CHANNEL</p><h1>{snapshot.room.code}</h1><p>{snapshot.room.category}频道 · {snapshot.room.difficulty}干扰</p><button type="button" onClick={() => void copyInvite()}><Copy size={17} />{copyStatus === '邀请链接已复制' ? '已复制' : '复制邀请链接'}</button>{copyStatus && <p className="copy-status" role="status">{copyStatus}</p>}</div>
      ...
      {me?.seat === 'A' && <button className="start-button waiting-start" disabled={busy || players.length < 2} onClick={() => credential && command(() => startGame(credential))}><span>{busy ? '正在抽取词语…' : '房主开始对决'}</span><ArrowRight /></button>}{actionError && <p className="error-message">{actionError}</p>}<button className="quit-button" onClick={leave}>退出房间</button></section>
    : <section className="game-stage"><div className="game-meta"><span>第 {snapshot.room.turnNumber} 回合</span><span>{snapshot.room.category}频道</span><span className={`network-${network}`}>{network === 'online' ? '已连接' : network === 'connecting' ? '连接中' : network === 'unstable' ? '网络波动' : '离线'}</span></div>
      <div className="arena"><PlayerPanel player={players.find((p) => p.seat === 'A')} seat="A" active={snapshot.room.status === 'playing' && current?.seat === 'A'} guesses={snapshot.guesses} me={me?.seat === 'A'} network={network} />
        <section className="tuning-console"><div className="turn-banner">{snapshot.room.status === 'playing' ? <>轮到 {current?.nickname}<span className={`timer ${seconds <= 8 ? 'urgent' : ''}`}>{seconds}s</span></> : '本局信号已锁定'}</div>
          <div className={`score-display ${(latest?.similarity || 0) >= 80 ? 'hot' : ''}`}><span className="score-label">{latest ? '最新猜词' : '等待首次猜词'}</span><div className="latest-word">{latest?.displayWord || '——'}</div><strong>{latest?.similarity || 0}<small>%</small></strong><div className="meter"><i style={{ width: `${latest?.similarity || 0}%` }} /></div><div className="meter-ticks"><span>微弱</span><span>接近</span><span>命中</span></div></div>
          {snapshot.room.status === 'playing' ? <form className="guess-form" onSubmit={sendGuess}><label htmlFor="guess">{snapshot.room.currentPlayerId === me?.id ? '输入你的猜测' : `等待 ${current?.nickname} 发射信号`}</label><div><input id="guess" disabled={busy || snapshot.room.currentPlayerId !== me?.id || seconds <= 0} value={guess} onChange={(e) => setGuess(e.target.value)} placeholder="一个中文词语" /><button disabled={busy || snapshot.room.currentPlayerId !== me?.id || !guess.trim()}>发射</button></div><p>倒计时和回合归属由服务端判定</p></form>
          : <div className="winner-card"><Trophy size={42} /><p>本局胜者</p><h2>{players.find((p) => p.id === snapshot.room.winnerId)?.nickname}</h2><strong>成功锁定：{snapshot.room.answer}</strong><button disabled={busy || Boolean(me?.rematchReady)} onClick={() => credential && command(() => requestRematch(credential))}><RotateCcw size={18} />{me?.rematchReady ? '已确认，等待对方' : '确认再来一局'}</button><small>{rematchReadyCount}/2 位玩家已确认</small><button className="quit-button" type="button" onClick={leave}>回到主页</button></div>}{actionError && <p className="error-message">{actionError}</p>}</section>
      ...
      <section className="history-board"><div className="history-heading"><div><span>公开波段</span><h2>猜测记录</h2></div><span>共 {sortedHistory.length} 次信号</span></div>{sortedHistory.length ? <div className="history-list">{sortedHistory.map((item) => { const player = players.find((p) => p.id === item.playerId); return <div className={`history-item p${player?.seat === 'A' ? 1 : 2}`} key={item.id}><span className="history-player">{player?.nickname}</span><strong>{item.displayWord}</strong><b>{item.similarity}%</b></div> })}</div> : <div className="empty-history">频道还很安静，第一条猜测会出现在这里。</div>}</section><button className="quit-button" onClick={leave}>退出房间（席位保留）</button></section>}
  </main>
}
